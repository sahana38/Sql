...
SELECT games_name, no_of_medals, sum(no_of_medals) as sum from common_wealth_games group by game_name having no_of_medals = 11;
...
SELECT games_name, no_of_players, min(no_of_players) as min from common_wealth_games group by no_of_players having min(no_of_players) > 10;
